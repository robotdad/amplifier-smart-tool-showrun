...
def validate(request, model):
    ...
    if target.get("kind") == "url":
        ...
        obj(target, {"kind", "python", "storage", "story_id", "revision_id"},
    ...
    require(authority["navigation_only"] is True and authority["disclose_dom"] is True,
            "Explicit navigation-only and DOM disclosure authority are required.")
    ...
    ids = set()
    for step in steps:
        obj(step, {"id", "instruction", "visible_text", "hold_seconds"}, {"id", "instruction", "visible_text"})
        ident(step["id"])
