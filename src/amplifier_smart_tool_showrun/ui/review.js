...
(function () {
  ...
  function updatePlayback() {
    ...
    toggle.setAttribute("aria-label", player.paused ? "Play recording" : "Pause recording");
  }
  ...
  // Repeated rows (clips named "Clip 1" in every take) need distinct accessible
  // names, so assistive technology and automation can tell identical controls apart.
  function editableName(scope, item, where = "") {
    const button = document.createElement("button");
    button.type = "button";
    button.className = "editable-name";
    button.textContent = item.name;
    button.title = "Rename " + scope;
    button.setAttribute("aria-label", "Rename " + scope + ": " + item.name + (where ? " · " + where : ""));
    button.onclick = () => {
  ...
  function renderTree() {
    ...
    demos.forEach((demo) => {
      ...
      libraryOrder(demo.takes, "added_at").forEach((take) => {
        ...
        heading.append(editableName("take", take, demo.name), status,
          iconButton("Delete take: " + take.name + " · " + demo.name, "trash", () => prepareDelete("take", take)));
        ...
        take.clips.forEach((clip) => {
          ...
          row.append(editableName("clip", clip, take.name + " · " + demo.name));
          const status = document.createElement("span");
          ...
          row.append(open, iconButton("Delete clip: " + clip.name + " · " + take.name + " · " + demo.name,
  ...
  function renderSelection({ reloadMedia = false } = {}) {
    ...
    for (const step of allSteps()) {
      ...
      const summary = String(step.requested?.instruction || step.id || "Unnamed step").replace(/\s+/g, " ").trim();
      jump.setAttribute("aria-label", `${jump.textContent} ${allSteps().indexOf(step) + 1}: ${summary.length > 80 ? summary.slice(0, 79) + "…" : summary}`);
  ...
  function bind() {
    ...
    $("play-toggle").onclick = () => {
      const player = $("player");
      if (player.paused) run(() => startPlayback()).catch(() => {});
      else { sectionEnd = null; player.pause(); }
    };
